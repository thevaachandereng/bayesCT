...
#'
#' @param total_sample_size integer. The total sample size.
#' @param prop_loss_to_followup integer. The proportion of loss to follow-up.
#' @param study_period integer. The length of the study (in days).
#' @param interim_look vector. Vector with interim looks (sample sizes).
#' @param .data NULL. This should not be changed by the user.
#'
#' @return A list with sample size, length of the study, interim looks and
#'   proportion loss to follow up.
#'
#' @examples study_details(total_sample_size = 300, study_period = 50,
#'                         interim_look = c(210, 240, 270))
#' @export study_details
study_details <- function(total_sample_size,
                          study_period,
                          interim_look          = NULL,
...
#' @param time vector. Vector with different cut-off times (knots) for lambda.
...
enrollment_rate <- function(lambda = 0.3,
                            time   = NULL,
                            .data  = NULL) {
  .data$lambda      <- lambda
...
#'
#' @param no_of_impute integer. Number of Monte Carlo imputations for missing
#'   data.
#' @param number_mcmc scalar. Number of Markov Chain Monte Carlo (MCMC) draws
...
#'   block.
#' @param randomization_ratio vector. The randomization allocation for control
#'   to treatment.
#' @inheritParams study_details
#'
...
#'
#' @param delta numeric. Threshold set for margin in null hypothesis. The
#'   default is set to 0.
...
#'   probability, probability of accepting the alternative hypothesis, and
